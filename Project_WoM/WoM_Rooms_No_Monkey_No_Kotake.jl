...
This file is dedicated to Day 2 WITHOUT the monkey and WITHOUT Kotake

The only differences between this file and WoM_Rooms_Monkey_No_Kotake.jl is that the monkey in Room 1
is cleared (so it attempts to allocate and immediately deallocates) and the two monkeys in Room 3
are cleared (so they both attempt to allocate and then deallocate) AND Koume is cleared
...
Room1_Scene_Load_Actor_List = [
    ...
    overlay_type="A",
    address=0,
    room_number=1, # room 1 monkey
    priority=1,
    unloadable=false,
    clearable=true,
    cleared=true, ################ The monkey is now cleared!
    attempts_to_allocate_if_cleared=true, # This is important!!
    transition=false,
    plane_copy=false,
    grabbable_actor=false,
    valid_superslide_list=false
